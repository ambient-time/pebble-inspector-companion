...
private enum class SignalPage(val title: String) {
    Conversation("Conversation"), History("History"), Settings("Settings")
}
...
@Composable
fun SignalScreen(station: SignalStation) {
    ...
    Column(Modifier.fillMaxSize().imePadding()) {
        ...
        if (selected != null) {
            ...
            SignalPage.Settings -> SignalConfiguration(state, station) { confirmation = it }
        }
    }
        ...
        item {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    OutlinedButton(onClick = station::survey, enabled = !state.busy && state.settings.enabled.isNotEmpty()) { Text("Survey") }
                ...
                Text(
                    ...
                    else "Survey sends ${state.settings.enabled.size} enabled ${if (state.settings.enabled.size == 1) "source" else "sources"} to ${providerLabel(state.settings.provider)}. Inspect and change sources in Settings.",
...
private fun SignalDetail(
    ...
    LazyColumn(
        ...
        item {
            ...
            SignalHeading(record.question)
            Text("${providerLabel(record.provider)} · ${record.model} · ${record.state}")
        ...
        item {
            FlowRow(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                OutlinedButton(onClick = onResume, enabled = !state.busy) { Text("Resume conversation") }
...
private fun SignalConfiguration(state: SignalState, station: SignalStation, onConfirm: (SignalConfirmation) -> Unit) {
    ...
    LazyColumn(
        ...
        item { SignalHeading("Answer provider") }
        item {
            ...
            Text("Test sends a small request without survey information. Provider charges may apply.", style = MaterialTheme.typography.bodySmall)
        }
        item {
        ...
        item {
            ...
            Text("Enable only what you want included in a survey. Each source is optional. Disabling a source clears active context; saved records remain locally viewable.")
        }
...
@Composable
private fun SignalToggle(label: String, checked: Boolean, enabled: Boolean = true, description: String? = null, onChange: (Boolean) -> Unit) {
...
@Composable
private fun SignalChoice(label: String, value: String, options: List<Pair<String, String>>, enabled: Boolean = true, onChange: (String) -> Unit) {
...
private fun providerLabel(value: String) = providerOptions.firstOrNull { it.first == value }?.second ?: value
private fun signalDateTime(value: Long): String = Instant.fromEpochMilliseconds(value)
